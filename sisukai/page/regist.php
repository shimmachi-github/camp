<?php 
    ...
    if (!empty($_POST)) {
        ...
        //メールアドレスの重複を検知
        if(!isset($error)){
            if(exist_check_email($post['email'], $db)){
        ...
        //パスワード正規表現チェック
        if(!isset($error)){
            if(!preg_match('/\A(?=.*?[a-z])(?=.*?[A-Z])(?=.*?\d)[a-zA-Z\d]{8,100}+\z/', $post['pass'])){
                $error['pass'] = 'preg';
            }
        }
        ...
        /* エラーがなければ次のページへ */
        if (!isset($error)) {
            $_SESSION['regipost'] = $post;   // フォームの内容をセッションで保存
            header('Location: regist_check.php');   // check.phpへ移動
            exit();
        }
    ...
    <div class="container">
        ...
        <div id="">
                <div class="regist-frame">
                        <div class="content regist">
                            <form action="" method="POST" autocomplete="off">
                                ...
                                <div class="control">
                                    ...
                                    <input id="name" type="text" name="name" value="<?php if(!empty($post['name'])){echo $post['name'];} ?>">
                                ...
                                <div class="control">
                                    ...
                                    <input id="email" type="email" name="email" value="<?php if(!empty($post['email'])){echo $post['email'];} ?>">
                                ...
                                <div class="control">
                                    ...
                                    <?php elseif (!empty($error["pass"]) && $error['pass'] === 'preg'):?>
                                        <p class="error">＊パスワードは ・8文字以上 ・英大文字 ・英小文字 ・数字 を最低一つずつ使用してください</p>
                                    <?php endif ?>
                                </div>
                        ...
                        <h2>会員登録済みの方は<a href="login.php">こちら</a></h2>
                    </div>
                </div>        
            </div>
            <footer>
